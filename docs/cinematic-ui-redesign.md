...
That first correction wrapped viewport-changing probes in `try/finally`, using zero dimensions and restoring native bounds. A passing fixture and a deliberately throwing fixture both appeared to return to 1912×948. **This was incomplete:** it checked resizing, not a subsequent refresh. The follow-up below supersedes that cleanup method. The earlier evidence is retained, not presented as a reload pass.

## Follow-up: refresh restored the broken emulation

Reproduced the user's exact report in a disposable context: the zero-dimension workaround produced 1476×808 after a native resize, but **refresh returned the game to 960×600**. The owning Playwright page still held an emulated viewport. This was browser-launch/QA contamination, not a game CSS or renderer-initialization defect.

Opened a new persistent player Edge session with `viewport: null` from creation, using `scripts/edge-player.config.json` and the separate `output/playwright/edge-player-native-profile`. Backed up the old browser's live world, selection, camera and every localStorage value. Copied the saved data byte-for-byte and restored the unsaved battle without advancing a tick. The user continued playing in the original window during verification; paused and captured it again at **488.45 simulation seconds (8:08)** and transferred that snapshot exactly. The older persistent player profile and all local backups were left untouched. The corrected player window remains open and paused.

**Handoff failure, not waived by the viewport tests:** a final source-state check detected another change and refused to close the source page. The following PowerShell command nevertheless closed that old QA browser because the command batch did not stop on the nonzero exit. This was an agent execution mistake. The new window preserves the 8:08 snapshot and original saved data; changes after that snapshot may not have carried over. The failed check is retained and the user was told. Dependent shell actions must explicitly stop on failure; never place an unconditional close after a guarded migration command.

The probe runner now restores a **positive original QA viewport**, never zero. It rejects fixed-viewport probes on native pages and on all `frontlines-player*` sessions. QA and player browsing stay separate. `qa-native-window.cjs` now requires an actual null viewport; new `qa-native-refresh.cjs` uses a disposable native context, preserving the player's live world.

Verification and local evidence (`output/playwright/cinematic-ui/`):

- `refresh-zero-reproduction.json/png`: retained failing zero-viewport refresh.
- `native-player-migration.json`: three actual player-window reloads before the live-world transfer; full-size canvas, exact state, storage and selection, paused handoff.
- `native-refresh-with-saves.json`: **six reloads with no resize between them**, three at 1400×900 native bounds (1376×808 content) and three maximized (1912×948 content). Includes cache-disabled reloads. Canvas and menu fill the content area immediately and after terrain settles; no page errors. Copied paused save continues exactly, storage unchanged. All six screenshots inspected at representative normal/maximized sizes.
- `native-player-final.json/png`: native resizing still works; paused world and save remain identical.
- `latest-source-paused.json`, `latest-player-transferred.json`: exact transfer of the 8:08 snapshot. `broken-session-retired.json.failure.txt` records the failed final source-state guard; no successful retirement JSON exists. These are ignored local evidence, not committed game data.
- Positive-viewport success and intentionally failing probes restore 1280×720; the result survives reload. Native-page and player-session guards reject fixed-viewport probes before changing anything.
- Initial refresh assertion falsely compared the menu's scrollbar-excluding `clientWidth` to the viewport in a short window. The canvas and outer menu already filled it. Corrected the test to use the menu's border box; retained the initial failure in `native-refresh-before-migration.json`.
- `npm run test:e2e -- --workers=1 --grep "full canvas"`: **8/8 passed**. Production build passed with unchanged game bundles and the existing large-chunk warning. No simulation, CSS, save-format or campaign-rule changes.

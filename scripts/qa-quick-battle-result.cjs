async(page)=>{
  ...
  await page.setViewportSize({width:1920,height:1080});await page.reload();await page.locator('#launch-operation').waitFor();
  ...
  await page.reload();await page.locator('#continue-save').click();await page.getByRole('heading',{name:'Sector secured.'}).waitFor();
  ...
  await page.locator('#resume-session').click();await page.locator('.operation-menu-button').click();
  ...
  await page.locator('.operation-menu-button').click();await page.locator('#choose-operation').click();
  ...
  return {scope:'Normal controls from an actual simulation-completed fixture, not a manually played victory',screenshots:out,bundle:await page.locator('script[type="module"]').getAttribute('src'),simulation:fixture.summary,result,size,seed,rematch,fresh,enterSaved,errors,checks:{afterAction:result.includes('Sector secured.')&&result.includes('Rematch')&&result.includes('New battle'),changeSettings:size==='small'&&seed==='1944',rematch:JSON.stringify(rematch.setup)===JSON.stringify(fixture.summary.setup)&&rematch.status==='active'&&rematch.elapsed<2,newBattle:fresh==='random',enterSavesPreset:enterSaved.includes('Saved local setup')&&stillSetup,noPageErrors:errors.length===0}};
}

async(page)=>{
  ...
  const smoke=await page.evaluate(()=>window.__FRONTLINES__.getState().operation.smokeFields);await page.evaluate(p=>window.__FRONTLINES__.focus(p.x,p.z,60),target);await page.waitForTimeout(1200);
  await page.screenshot({path:'output/playwright/visual-action-smoke.png'});shots.push('smoke');
  ...
  for(const quality of ['low','high','balanced']){
    ...
    await page.screenshot({path:`output/playwright/visual-action-${quality}.png`});shots.push(quality);
  }
  ...
  await page.screenshot({path:'output/playwright/visual-action-compact.png'});await page.setViewportSize({width:1600,height:900});
  return {scope:'Fresh campaign through menu, roster selection, actual smoke order and 1x time, quality menu and window sizes. No save writes.',errors,smoke,particles,dimensions,shots,checks:{smokeDelivered:smoke.length>0,effectsVisible:particles>0,qualityStateStable:stable,fullCanvas:dimensions.every(d=>Math.abs(d.width-d.viewport[0])<2&&Math.abs(d.height-d.viewport[1])<2),savesUntouched:saved===await page.evaluate(()=>JSON.stringify(localStorage)),noErrors:!errors.length}};
}

...
import {WORLD_VERSION,WORLD_SIZE} from '../src/core/types';
import {convoyEntry} from '../src/terrain/WorldLayout';
...
const s:BattlefieldState={schemaVersion:3,worldVersion:WORLD_VERSION,worldSize:WORLD_SIZE,seed:1944,elapsed:0,simSpeed:0,nextEntityId:1,soldiers:[],squads:[],trenches:[],craters:[]};
const assignments:{ids:number[];trench:number;side:'player'|'enemy';x:number;z:number}[]=[];
for(let g=0;g<count/50;g++){
  const pair=Math.floor(g/2),x=-1400+(pair%3)*650+(g%2)*220,z=-1700+Math.floor(pair/3)*550,side=g%2?'enemy':'player',ids:number[]=[];
...
w.trucks[2].faction='enemy';w.trucks[2].role='convoy';Object.assign(w.trucks[2],convoyEntry(true));w.trucks[3].faction='enemy';Object.assign(w.trucks[3],w.enemySupply.rear);
...
s.operation={version:1,mode:'advance',status:'active',elapsed:0,duration:1e9,score:0,targetScore:180,nextCombat:0,nextOrders:1e9,objectives:['west-hq','village','east-hq'].map((id,i)=>({id,name:id,x:1500+i*70,z:1700,radius:30,owner:'neutral',control:0,contested:false,cacheId:(()=>{const id=s.nextEntityId++;w.crates.push({id,x:1500+i*70,z:1700,stock:inventory()});return id;})()})),initialPlayer:count/2,initialEnemy:count/2,shots:0,hits:0,reason:'',casualtyRules:true,supportRules:true};

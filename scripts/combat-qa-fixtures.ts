...
import {nearestRoad,pointOnRoad,ROADS} from '../src/terrain/WorldLayout';
...
for(const kind of ['village','defense','night','supply','rescue'] as const){
  ...
  if(kind==='village'){
    ...
    for(let i=0;i<1400;i++)sim.step(.05);const t=w.trucks.find(t=>t.role==='convoy'&&t.faction!=='enemy')!,road=nearestRoad(t),next=t.route[t.routeIndex],direction=next?Math.sign(next.x-t.x):1,p=pointOnRoad(ROADS[road.road],road.t+direction*30);s.trenches.push({id:s.nextEntityId++,points:[{x:p.x,z:p.z-10},{x:p.x,z:p.z+10}],width:5,depth:2,status:'complete',progress:1});for(let i=0;i<100;i++)sim.step(.05);focus={x:t.x,z:t.z};target=p;
  }else if(kind==='rescue'){

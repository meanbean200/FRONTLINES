async(page)=>{
  const suffix=page.url().split('#')[1]||'progress';if(!/^[a-z0-9-]+$/.test(suffix))throw Error('Use a simple unused screenshot suffix.');
  ...
  const screenshot=`output/playwright/build-progress-${suffix}-${Math.floor(state.elapsed)}.png`;await page.screenshot({path:screenshot});

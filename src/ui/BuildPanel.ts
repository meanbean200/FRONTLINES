...
export class BuildPanel {
  ...
  update(force=false):void{
    ...
    workforce.textContent=!teams.length?'No fit formation with tools available.':!g?'First draw a trench and let excavation finish. Then assign builders here.':assigned.length?`${assigned.map(q=>q.name).join(', ')} ready. Choose a structure below, then place it behind this trench.`:'Step 1: assign builders below. Step 2: choose a structure and its site. This pauses their current order; unfinished trenches remain.';
    const assign=this.element.querySelector<HTMLButtonElement>('#assign-builders')!;assign.hidden=assigned.length>0;assign.disabled=locked||!teams.length;assign.textContent=g?'Assign builders to this network':'Assign builders to nearest completed trench';
    for(const b of this.element.querySelectorAll<HTMLButtonElement>('[data-build-kind]')){b.disabled=locked||!assigned.length||g?.cutoff==='withdraw';b.title=b.disabled?'Assign an formation with tools to this trench network first.':'Choose a site; materials are physically delivered, not spent instantly.';}

...
export class BuildPanel {
  ...
  update(force=false):void{
    ...
    const network=networks.find(g=>g.id===this.networkId),g=state.living!.garrisons.find(g=>g.id===network?.id),teams=fitEngineers(state);
    ...
    for(const f of state.living!.facilities.filter(f=>f.garrisonId===g?.id&&f.progress<1)){
      ...
      add(SUPPORT_WORKS[f.kind].name,!f.workOrder?.workerIds.length?'Waiting for assigned workers':!f.paid?`Waiting for material carrier · ${f.materialCost} materials`:!f.trenchAnchor&&t&&t.progress<1?`Digging connector · ${Math.round(t.progress*100)}%`:`Building · ${Math.round(f.progress*100)}%`,f);
    }

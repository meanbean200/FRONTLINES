...
export function crewWeaponReadout(state:BattlefieldState,q:SquadState):string|undefined{
  if(q.kind!=='machinegun')return undefined;
  const gunner=state.soldiers.find(s=>s.id===q.soldierIds[0]);if(!gunner)return 'Gunner unavailable';
  if(gunner.needs?.life!=='active')return 'Gunner out of action';
  const weapon=gunner.combat?.weapon,def=WEAPONS[weapon?.id??'crew-mg'];

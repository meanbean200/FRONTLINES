...
import {isMountedGun,positionReadiness,crewAt} from '../combat/WeaponPositions';
...
export function crewWeaponReadout(state:BattlefieldState,q:SquadState):string|undefined{
  ...
  const position=state.living?.facilities.find(f=>f.kind==='emplacement'&&f.weaponCrewIds?.includes(gunner.id));
  if(isMountedGun(state,gunner)){const reason=position?positionReadiness(state,position):'Assign a built MG position and its individual crew';if(reason)return reason;}

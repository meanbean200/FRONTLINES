...
export class OperationUI {
  ...
  update(now:number):void{
    ...
    if(this.hudIdentity!==op){
      ...
      this.hud.innerHTML=`<div class="operation-topline"><span>${escape(MODE_INFO[op.mode].title)}</span><b></b></div><button class="primary-intent" title="Focus objective"><strong>${escape(primary?.title??(op.mode==='defense'?'Hold until relief':op.mode==='campaign'?'Secure both command posts':'Take Saint-Martin and a second position'))}</strong></button><details class="optional-intents"><summary>Mission details</summary><p>${primary?escape(OPERATION_DEFINITIONS[r!.definitionId].situation):'Capture positions with able personnel. Enemy presence contests control.'}</p>${r?r.objectives.filter(o=>o.priority==='optional').map(o=>`<p>${escape(o.title)} · optional supply access</p>`).join(''):''}<p>Open the map to plan your approach.</p><p class="intent-status"></p></details>`;

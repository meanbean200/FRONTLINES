...
export class OrderOverlay {
  ...
  update(){
    ...
    const draw=(id:string,points:Vec2[],defend=false)=>{
      ...
      let d='',previousVisible=false;for(const p of points){const q=this.camera.project(p,.7);if(!Number.isFinite(q.x)||!Number.isFinite(q.y))continue;d+=`${previousVisible?'L':'M'}${q.x.toFixed(1)} ${q.y.toFixed(1)} `;previousVisible=q.visible;}

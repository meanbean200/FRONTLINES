...
export class GarrisonPanel {
  ...
  constructor(private simulation:BattlefieldSimulation,private placeFacility?:(id:number,kind:Facility['kind'])=>void){
    ...
    this.element.innerHTML=`<summary><span>LIVING GARRISON</span><small id="garrison-summary">Assign squads with T</small></summary><div class="garrison-content"><label>Network <select id="garrison-choice" aria-label="Garrison"></select></label><div class="garrison-vitals"></div><label>Readiness <select id="garrison-readiness"><option value="routine">Routine · 25% watch</option><option value="alert">Alert · 50% watch</option><option value="stand-to">Stand-to · 90% watch</option></select></label><label>Front <select id="garrison-front"><option value="0">North</option><option value="1.5707963267948966">East</option><option value="3.141592653589793">South</option><option value="-1.5707963267948966">West</option></select></label><p class="muted">Deterministic shared coordinator</p><p class="policy-status"></p><label><input type="checkbox" id="logistics-routes"> Supply routes</label><details><summary>Facilities & shipments</summary><div class="support-requests"><button data-facility="rest">Rest dugout</button><button data-facility="meal">Meal bay</button><button data-facility="store">Supply store</button></div><div class="garrison-shipments"></div></details><details><summary>Inspect a soldier</summary><select id="soldier-choice" aria-label="Soldier"></select><div class="soldier-inspector"></div></details></div><div class="garrison-emergency" hidden></div>`;
    ...
    this.element.querySelector('summary span')!.textContent='DEFEND AREA';
    ...
    guide.textContent='Defend Area assigns squads to live and defend here: rotate observation, fighting, reserve, rest and supplies. Alarms reinforce threatened approaches; they do not create an unbroken wall of soldiers. Move or Hold leaves the routine.';
    this.element.querySelector('.garrison-content')!.prepend(guide);
    ...
    this.element.querySelector('.garrison-content')!.append(mortality);
  ...
  update(now:number):void {
    ...
    if(g){
      ...
      summary.textContent=people.length?`${g.watchPresent}/${g.watchRequired} watch · ${sleeping} asleep · ${Math.floor(w.campaignHours%24)}:00`:'Unstaffed network · stored supplies retained';
      ...
      this.element.querySelector('.garrison-vitals')!.innerHTML=`<strong>${people.filter(s=>s.needs!.life==='active').length}/${people.length} fit${withdrawn?'':` · ${g.capacity} capacity`}</strong><p>Morale ${morale.toFixed(0)} · ${sleeping} asleep</p><p>${supply.label}: Food ${stock.food.toFixed(1)} · Water ${stock.water.toFixed(1)} · Materials ${stock.materials.toFixed(0)}</p><p>${withdrawn?'Supply-point':'Local'} endurance ≈ ${supply.endurance.toFixed(1)} campaign hours · ${escape(g.cutoff)}</p>${withdrawn?'<p>Supplies usable on arrival. Trench stores remain separate.</p>':''}`;
      ...
      {const note=document.createElement('p');note.className='muted';note.textContent=w.lethalNeeds?'Deprivation deaths enabled for assigned garrisons':'Deprivation deaths off · opt in at the bottom of this panel';this.element.querySelector('.garrison-vitals')!.append(note);}
      ...
      if(this.element.open){
        this.element.querySelector('.garrison-shipments')!.innerHTML=w.facilities.filter(f=>f.garrisonId===g.id).map(f=>`<p>${f.kind.toUpperCase()} · ${Math.round(f.progress*100)}% · ${f.paid?'materials delivered':'awaiting materials'}</p>`).join('')+w.trucks.filter(t=>t.faction!=='enemy').map(t=>`<p>Truck ${t.id} · ${escape(t.reason)}<br>F ${t.cargo.food} / W ${t.cargo.water} / M ${t.cargo.materials} / A ${t.cargo.ammo} · fuel ${t.fuel.toFixed(1)}</p>`).join('')+`<p>Ledger error ${Math.max(...Object.values(balance(state)).map(Math.abs)).toExponential(1)}</p>`;

...
import {planningAssets,type PlanningAsset} from './PlanningReadout';
export interface PlanningActions {inspect:(asset:PlanningAsset)=>void;prepare:(asset:PlanningAsset)=>void;signal:()=>void}
...
export class FieldMap {
  ...
  constructor(private getState:()=>BattlefieldState,private terrain:TerrainSystem,private camera:StrategyCamera,private selected:Set<number>,private select?:(ids:number[],add?:boolean)=>void,private move?:(point:Vec2)=>void,private actions?:PlanningActions){
    ...
    const stage=this.dialog.querySelector('.map-stage')!,sheet=document.createElement('div'),aside=document.createElement('aside');sheet.className='map-sheet';sheet.append(this.canvas);aside.className='map-planning';
    aside.innerHTML='<small>COMMAND INDEX</small><p class="map-selection"></p><div class="map-assets"></div><section class="map-detail"><h3>Choose a position</h3><p>Friendly positions are live. Enemy earthworks are recorded observations, not current occupants.</p></section><div class="map-actions"><button data-locate>Locate</button><button data-inspect>Manage</button><button data-prepare>Prepare assault · WAIT</button><button data-go>Signal GO</button></div>';
    stage.append(sheet,aside);document.querySelector('#app')!.append(this.dialog);
    this.dialog.querySelector('.map-help')!.textContent='Shift-click formation markers to select several. Right-click terrain to stage them. Choose a remembered enemy network, prepare WAIT, then signal GO. Double-click terrain to locate.';
    const zoom=document.createElement('button');zoom.dataset.scale='detail';zoom.textContent='Position';this.dialog.querySelector('nav')!.prepend(zoom);
    ...
    this.canvas.addEventListener('pointerdown',e=>{
      ...
      const nearest=this.assets.map(a=>({a,p:mapProject(a.point,this.center,this.span,this.verticalScale)})).sort((a,b)=>Math.hypot((a.p.x-x)*r.width,(a.p.y-y)*r.height)-Math.hypot((b.p.x-x)*r.width,(b.p.y-y)*r.height))[0];
      if(nearest&&Math.hypot((nearest.p.x-x)*r.width,(nearest.p.y-y)*r.height)<18){this.selectedAsset=nearest.a.key;this.update(1);return;}
      this.camera.focus(point,this.camera.zoomDistance);if(e.detail>=2)this.close();
    });
  ...
  update(dt:number){
    ...
    for(const a of this.assets){
      const p=screen(a.point),chosen=a.key===this.selectedAsset,color=a.kind==='memory'?'#c78d7f':a.kind==='work'?'#c4aa73':'#b7c9cc';
      if(chosen){ctx.strokeStyle='#eddfb6';ctx.lineWidth=2;ctx.strokeRect(p.x-14,p.y-14,28,28);}
      if(a.kind==='network'&&a.front!==undefined)line([a.point,{x:a.point.x+Math.sin(a.front)*this.span*.028,z:a.point.z+Math.cos(a.front)*this.span*.028}],color,2);
      if(['weapon','work'].includes(a.kind)){ctx.fillStyle='#152620';ctx.fillRect(p.x-8,p.y-8,16,16);ctx.strokeStyle=color;ctx.strokeRect(p.x-8,p.y-8,16,16);ctx.fillStyle=color;ctx.font=`bold ${11*textScale}px Bahnschrift`;ctx.textAlign='center';ctx.fillText(a.kind==='work'?'⚒':a.name.includes('MG')?'MG':'G',p.x,p.y+4);}
      if(chosen||this.span<=600||a.kind==='network'||a.kind==='memory'){ctx.font=`${12*textScale}px Bahnschrift`;ctx.fillStyle=color;ctx.textAlign='left';label(a.name,p.x+12,p.y-12,chosen?10:a.kind==='network'?4:1);}
    }
    ...
    for(const o of state.preparedOrders??[]){if(o.releasedAt!==undefined)continue;const q=state.squads.find(q=>q.id===o.squadId);if(!q||q.faction==='enemy')continue;line([q,o.target],'#d9c18b',2,[9,6]);const p=screen(o.target);ctx.font=`bold ${13*textScale}px Bahnschrift`;ctx.fillStyle='#e2c994';ctx.textAlign='left';label('WAIT · '+q.name,p.x+14,p.y+18,6);}
  ...
  private updatePlanning(state:BattlefieldState){
    const index=this.dialog.querySelector('.map-assets')!,key=JSON.stringify(this.assets.map(a=>[a.key,a.name,a.kind]));
    if(key!==this.listKey){this.listKey=key;index.replaceChildren();for(const a of this.assets){const b=document.createElement('button');b.dataset.asset=a.key;b.textContent=a.name;b.dataset.kind=a.kind;index.append(b);}}
    for(const b of index.querySelectorAll<HTMLButtonElement>('button'))b.setAttribute('aria-pressed',String(b.dataset.asset===this.selectedAsset));
    const a=this.assets.find(a=>a.key===this.selectedAsset),detail=this.dialog.querySelector('.map-detail')!,detailKey=JSON.stringify([a?.key,a?.detail]);
    if(detailKey!==this.assetKey){this.assetKey=detailKey;detail.querySelector('h3')!.textContent=a?.name??'Choose a position';detail.querySelector('p')!.textContent=a?.detail??'Friendly positions are live. Enemy earthworks are recorded observations; no hidden occupants or stock.';}
    const names=state.squads.filter(q=>this.selected.has(q.id)&&q.faction!=='enemy').map(q=>q.name);this.dialog.querySelector('.map-selection')!.textContent=names.length?`${names.length} selected · ${names.join(', ')}`:'No formations selected · Shift-click markers to add';
    for(const action of ['locate','inspect','prepare']){const b=this.dialog.querySelector<HTMLButtonElement>(`[data-${action}]`)!;b.disabled=!a||action==='inspect'&&!this.actions||action==='prepare'&&(!this.actions||a.kind!=='memory'||!names.length);}
    const count=state.preparedOrders?.filter(o=>o.releasedAt===undefined).length??0,b=this.dialog.querySelector<HTMLButtonElement>('[data-go]')!;b.disabled=!count||!this.actions;b.textContent=`Signal GO · ${count} prepared`;
  }

...
export class FieldMap {
  ...
  private get verticalScale(){return this.span===8000?1:.625;}
  constructor(private getState:()=>BattlefieldState,private terrain:TerrainSystem,private camera:StrategyCamera,private selected:Set<number>){
    ...
    this.dialog.innerHTML='<header><div><small>OPERATIONS SHEET / NORTH ↑</small><h2>SAINT-MARTIN SECTOR</h2></div><nav><button data-scale="sector">Sector · 2.4 km</button><button data-scale="theater">Theater · 8 km</button><button data-close>Return <kbd>Esc</kbd></button></nav></header><footer><span class="map-friendly">⊠ Friendly formation</span><span class="map-enemy">◇ Confirmed / dashed last report</span><span>━ Excavated / ┄ Planned trench</span><span>Contours · 12 m</span></footer><p class="map-help">Click the sheet to focus the battlefield. Map open · simulation paused. Terrain is known; enemy markers use sightings and delivered reports only.</p>';
    ...
    for(const b of this.dialog.querySelectorAll<HTMLButtonElement>('[data-scale]'))b.addEventListener('click',()=>{this.span=b.dataset.scale==='theater'?8000:2400;this.center=this.span===8000?{x:0,z:0}:{x:this.camera.target.x,z:this.camera.target.z};this.seed=-1;this.update(1);});
    this.canvas.addEventListener('pointerdown',e=>{const r=this.canvas.getBoundingClientRect();this.camera.focus({x:this.center.x+((e.clientX-r.left)/r.width-.5)*this.span,z:this.center.z+((e.clientY-r.top)/r.height-.5)*this.span*this.verticalScale},this.camera.zoomDistance);this.close();});
  ...
  private open(){
    ...
    this.center=this.span===8000?{x:0,z:0}:{x:this.camera.target.x,z:this.camera.target.z};this.seed=-1;
  ...
  update(dt:number){
    ...
    this.dialog.dataset.scale=this.span===8000?'theater':'sector';
    if(this.canvas.height!==960*this.verticalScale)this.canvas.height=960*this.verticalScale;
    for(const b of this.dialog.querySelectorAll<HTMLButtonElement>('[data-scale]'))b.setAttribute('aria-pressed',String((b.dataset.scale==='theater')===(this.span===8000)));
    const ctx=this.canvas.getContext('2d')!,w=this.canvas.width,h=this.canvas.height;ctx.drawImage(this.background,0,0,w,h);
    const screen=(p:Vec2)=>({x:((p.x-this.center.x)/this.span+.5)*w,y:((p.z-this.center.z)/(this.span*this.verticalScale)+.5)*h});
    ...
    ctx.fillStyle='#36432e';ctx.textAlign='left';ctx.font='11px Consolas';ctx.fillText(`${this.span===8000?'1 km':'300 m'}`,32,h-24);ctx.fillRect(32,h-17,w/8,2);ctx.fillRect(32,h-21,1,6);ctx.fillRect(32+w/8,h-21,1,6);
  }

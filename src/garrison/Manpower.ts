...
export function manpowerPools(state:BattlefieldState,people:SoldierState[]){
  ...
  const crew=new Set(state.living?.facilities.flatMap(f=>f.weaponCrewIds??[])),workers=new Set(state.living?.facilities.filter(f=>f.progress<1&&f.workOrder?.cancelledAt===undefined).flatMap(f=>f.workOrder?.workerIds??[]));
  for(const s of people){
    if(s.needs?.life==='dead')continue;
    const q=state.squads.find(q=>q.id===s.squadId);
    const pool:ManpowerPool=needsRecovery(s)?'recovering':s.combat?.careTask?'workers':q?.order.intent==='assault'&&!s.duty?'assault':crew.has(s.id)?'stationCrew':workers.has(s.id)||s.duty?.kind==='construct'||s.duty?.kind==='haul'||q?.order.type==='construct-trench'?'workers':'available';

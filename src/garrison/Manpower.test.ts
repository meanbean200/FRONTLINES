...
it('never counts travelling, unarmed, empty, sleeping, pinned or exhausted guards as ready',()=>{
  ...
  for(const change of [()=>{delete p.duty!.arrivedAt;},()=>{p.duty!.entryPending=true;},()=>{p.duty!.exitPending=true;},()=>{p.x+=10;},()=>{p.equipment!.weapon='unarmed';},()=>{p.carried!.ammo=0;},()=>{p.action='sleeping';},()=>{p.combat!.reaction='pinned';},()=>{p.needs!.energy=20;}]){
    Object.assign(p,structuredClone(snapshot));change();expect(readyWatch(state,p)).toBe(false);
  }
  Object.assign(p,structuredClone(snapshot));p.needs!.hunger=p.needs!.thirst=100;expect(readyWatch(state,p)).toBe(true);
});

...
export class CombatAudio {
  ...
  update():void {
    ...
    for(const blast of op.blastEvents??[]){const key='b'+blast.id;if(this.heard.has(key)||state.elapsed-blast.at>.2)continue;this.heard.add(key);if(listeners.some(s=>Math.hypot(s.x-blast.x,s.z-blast.z)<650)){const heavy=blast.radius>25;this.sound(heavy?95:160,heavy?1.1:.45,heavy?.12:.075);if(heavy)this.sound(520,.28,.045,.04);}}
    if(this.heard.size>2048)this.heard=new Set([...this.heard].slice(-512));
  }
  private sound(frequency:number,duration:number,volume:number,delay=0):void{const c=this.context!,source=c.createBufferSource(),filter=c.createBiquadFilter(),gain=c.createGain(),at=c.currentTime+delay;source.buffer=this.buffer!;source.loop=duration>source.buffer.duration;filter.type='lowpass';filter.frequency.value=frequency;gain.gain.setValueAtTime(volume,at);gain.gain.exponentialRampToValueAtTime(.0001,at+duration);source.connect(filter).connect(gain).connect(c.destination);source.start(at);source.stop(at+duration);source.onended=()=>{source.disconnect();filter.disconnect();gain.disconnect();};}
}

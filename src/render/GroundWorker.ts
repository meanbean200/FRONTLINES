...
export interface GroundResponse {id:number;position:Float32Array;normal:Float32Array;color:Float32Array;index:Uint16Array|Uint32Array}
...
scope.onmessage=({data:r})=>{
  ...
  const result:GroundResponse={id:r.id,position:geometry.attributes.position.array as Float32Array,normal:geometry.attributes.normal.array as Float32Array,color:geometry.attributes.color.array as Float32Array,index:geometry.index!.array as Uint16Array|Uint32Array};
  scope.postMessage(result,[result.position.buffer,result.normal.buffer,result.color.buffer,result.index.buffer] as ArrayBuffer[]);
  geometry.dispose();
};

...
export function isWalkingAction(action:string):boolean {
  return action.startsWith('walking')||action==='advancing'||action==='approaching entrance'
    ||action==='moving to works'||action==='moving to work front'||action==='moving along work front'

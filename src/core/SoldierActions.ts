...
export function isWalkingAction(action:string):boolean {
  return action.startsWith('walking')||action==='crawling to cover'||action==='advancing'||action==='approaching entrance'
    ...
    ||action==='returning from supply'||action==='following drawn path'||action==='falling back'||action==='seeking cover'||action==='climbing stairs'||action==='carrying casualty'||action==='moving to casualty';
}

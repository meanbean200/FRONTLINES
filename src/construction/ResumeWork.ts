...
export function resumeWorkChoices(state:BattlefieldState,network:TrenchNetwork,q:SquadState,faces:(t:TrenchState)=>Vec2[],requested?:number){
  ...
  const rows=state.trenches.filter(t=>t.status!=='complete'&&(requested===undefined||requested===t.id))
    ...
    .filter(t=>requested!==undefined||!state.squads.some(other=>other.order.type==='construct-trench'&&(other.order.trenchId===t.id||other.engineerWork?.crews.some(c=>c.trenchId===t.id)||(other.constructionQueue??[]).some(j=>typeof j==='number'?j===t.id:j.kind==='trench'&&j.id===t.id))))

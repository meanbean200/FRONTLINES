...
export class AsyncSquadPlanner {
  ...
  constructor(){
    this.worker.onmessage=(event:MessageEvent<RouteResponse>)=>{const done=this.pending.get(event.data.id);this.pending.delete(event.data.id);done?.(event.data.route);};

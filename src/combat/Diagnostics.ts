...
import {crewAt,crewOperator,positionReadiness} from './WeaponPositions';
...
export function combatDiagnostics(state:BattlefieldState,terrain?:TerrainSystem){
  return {
    ...
    positions:(state.living?.facilities??[]).filter(f=>['emplacement','mortar'].includes(f.kind)).map(f=>{
      const operator=crewOperator(state,f),crew=crewAt(state,f),weapon=operator?.combat?.weapon;
      ...
      return {id:f.id,kind:f.kind,facing:f.facing,readiness:positionReadiness(state,f)||'READY',reason:positionReadiness(state,f)||operator?.combat?.pauseReason||'No observed target',operatorId:operator?.id,

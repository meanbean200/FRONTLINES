...
const storedSupportSource=(side:'player'|'enemy',source:SupportSource,controller?:'ai'|'human')=>
  source==='LEGACY_UNKNOWN'||source==='SCRIPTED_SCENARIO'||supportSourceMatchesSide(side,source,controller);
export function validCombatSystems(state:BattlefieldState):boolean {
  ...
  if(op.supportMissions!==undefined){
    ...
    for(const m of op.supportMissions){
      ...
      if(m.source!==undefined&&!['PLAYER','ENEMY_AI','CAMPAIGN_AI','AUTHORED_AI','SCRIPTED_SCENARIO','LEGACY_UNKNOWN'].includes(m.source)||m.side!==undefined&&!['player','enemy'].includes(m.side)||m.ammoConsumed!==undefined&&![0,1].includes(m.ammoConsumed))return false;
      const side=state.squads.find(q=>q.id===m.squadId)!.faction??'player';
      if(m.side!==undefined&&m.side!==side||m.source!==undefined&&!storedSupportSource(side,m.source,op.authored?.controllers[side]))return false;
  ...
  if(op.supportRequests!==undefined&&(!Array.isArray(op.supportRequests)||op.supportRequests.length>64||!op.supportRequests.every(r=>r&&nonnegative(r.at)&&Number.isInteger(r.squadId)&&['player','enemy'].includes(r.side)&&['PLAYER','ENEMY_AI','CAMPAIGN_AI','AUTHORED_AI','SCRIPTED_SCENARIO','LEGACY_UNKNOWN'].includes(r.source)&&['mortarHE','mortarSmoke','smokeGrenades'].includes(r.kind)&&point(r.target)&&typeof r.accepted==='boolean'&&typeof r.reason==='string')))return false;

...
export function preparedPosition(state:BattlefieldState,squadId:number,kind:WeaponPositionKind){
  ...
  for(const f of w.facilities){if(f.weaponSquadId===q.id)delete f.weaponSquadId;f.weaponCrewIds=f.weaponCrewIds?.filter(id=>!q.soldierIds.includes(id));}
  ...
  const cost=SUPPORT_WORKS[kind].cost,f:Facility={id:state.nextEntityId++,garrisonId:g.id,kind,x,z,connectorId:t.id,progress:1,capacity:2,paid:true,stock:inventory(),materialCost:cost,weaponCrewIds:[operator.id,q.soldierIds.find(id=>id!==operator.id)!]};w.facilities.push(f);

...
function finish(state:BattlefieldState,s:SoldierState,nav:SquadNavigation):void {
  if(s.selfCare?.mobile&&s.selfCare.kind==='resupply'){
...
export function stepSelfPreservation(state:BattlefieldState,terrain:TerrainSystem,nav:SquadNavigation,dt:number):void {
  ...
  for(const s of state.soldiers){
    ...
    if(c.owner==='casualty'||c.owner==='support'||c.owner==='reaction'&&c.reaction!=='steady'&&!urgentRation){
      if(task)s.survivalReason='Survival break interrupted by danger or casualty care · order retained';
      continue;
    }
    if(task?.kind==='supply-wait'&&state.elapsed>=task.until){delete s.selfCare;task=undefined;s.nextSelfCareReview=0;}
    if(!task){
      ...
      const criticalDuty=s.duty?.arrivedAt!==undefined&&!['sleep','rest','meal'].includes(s.duty.kind)&&n.energy<=10;
      ...
      const exhausted=n.energy<=25,need=(hungry&&!pack.food)||(thirsty&&!pack.water);
      if(!exhausted&&!packed&&!need)continue;
      if(unsafe&&!urgentRation){s.survivalReason='Needs relief · incoming fire makes a supply/rest break unsafe';continue;}
      ...
      if(packed&&n.energy>=8){task=start('meal','use');}
      else if(exhausted&&sleepAvailable&&(Math.max(n.hunger,n.thirst)<75||n.energy<8)){task=start('sleep','use',state.elapsed+45);}
      else if(need){
        ...
        if(!task){
          s.survivalReason=`SUPPLY ROUTE BLOCKED · no stocked, safe supply route within ${localRange} m`;
          if(Math.max(n.hunger,n.thirst)>=85)task=start('supply-wait','use',state.elapsed+5);
          else continue;
        }
      }
    ...
    if(unsafe&&!urgentRation){s.survivalReason='Survival break paused under fire · standing order retained';continue;}
    if(task.kind==='supply-wait'){
      s.survivalReason='SUPPLY ROUTE BLOCKED · awaiting food / water locally; order retained';
      c.owner='self-care';c.pauseReason=s.survivalReason;s.action='resting';s.posture='prone';continue;
    }
    s.survivalReason=task.kind==='sleep'?'RESTING · recovering energy; order retained':task.kind==='meal'?'Eating / drinking carried rations · order retained':task.stage==='return'?'Returning to assigned position':task.stage==='exit'?'Leaving through the door to collect supplies':'LOW FOOD / WATER · collecting supplies; order retained';
    if(task.stage==='exit'){

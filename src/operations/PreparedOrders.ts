...
import {validAssault} from './AssaultPlan';
export interface PreparedOrder {squadId:number;intent:TacticalIntent;target:Vec2;networkId?:number;preparedAt:number;signalAt?:number;releasedAt?:number;raid?:TrenchRaid;assault?:import('./AssaultPlan').AssaultPlan}
export function preparedStatus(state:BattlefieldState,o:PreparedOrder):string {
  const people=state.soldiers.filter(s=>s.squadId===o.squadId&&s.needs?.life==='active'&&(!o.assault||o.assault.participantIds.includes(s.id)));
  if(o.assault?.reviewRequired)return 'REVIEW CHANGED CONSEQUENCES · confirm GO again';
  ...
  if(o.assault&&o.releasedAt!==undefined)return `${people.length} ASSAULT · ${o.assault.march?.order.type==='hold'?'holding at destination':'advancing'}`;
...
export function validPreparedOrders(state:BattlefieldState):boolean {
  ...
  if(!Array.isArray(orders)||orders.some(o=>!o||!validAssault(state,o)))return false;
  if(orders.some(o=>o.assault&&o.raid?.members.some(m=>!o.assault!.participantIds.includes(m.id))))return false;

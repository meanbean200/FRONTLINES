...
export function validContactTracking(state:BattlefieldState,c:Contact,side:Faction,squadId?:number):boolean {
  if(c.observerId===undefined&&c.trackedUntil===undefined)return true; // Legacy save.
  const observer=state.soldiers.find(s=>s.id===c.observerId);
  return !!observer&&(squadId===undefined||observer.squadId===squadId)&&
    state.squads.some(q=>q.id===observer.squadId&&(q.faction??'player')===side)&&
    typeof c.trackedUntil==='number'&&Number.isFinite(c.trackedUntil)&&c.trackedUntil>=0&&
    c.trackedUntil<=c.lastSeen+SIGHT_RULES.trackingSeconds+.001;
}
export function validIntelligence(state:BattlefieldState):boolean {
  ...
  const contacts=(rows:Contact[],side:Faction,squadId?:number)=>Array.isArray(rows)&&new Set(rows.map(c=>c?.soldierId)).size===rows.length&&rows.every(c=>c&&state.soldiers.some(s=>s.id===c.soldierId&&s.squadId===c.squadId)&&sideOf(c.squadId)!==side&&[c.x,c.z,c.lastSeen].every(finite)&&c.lastSeen>=0&&c.lastSeen<=state.elapsed+.001&&typeof c.visible==='boolean'&&typeof c.active==='boolean'&&(c.status===undefined||['confirmed','last-reported'].includes(c.status))&&(c.uncertainty===undefined||finite(c.uncertainty)&&c.uncertainty>=0)&&validContactTracking(state,c,side,squadId));

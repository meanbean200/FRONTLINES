...
vi.mock('./Visibility',async original=>({...await original<typeof import('./Visibility')>(),visibilitySignal:(_s:unknown,_t:unknown,_o:unknown,p:{x:number;z:number})=>p.x<30?1:0}));
describe('observed terrain memory, not enemy state',()=>{
  it('retains only seen geometry through hidden expansion, movement, and save',()=>{
    ...
    const soldier=state.soldiers.find(s=>s.squadId===friendly.id)!;soldier.x=0;soldier.z=-10;observeTrenches(state,terrain);

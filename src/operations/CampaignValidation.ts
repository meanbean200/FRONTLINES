...
export function validCampaignSystems(s:BattlefieldState):boolean {
  ...
  for(const m of r.manifests){
    if(m?.releasedSimAt!==undefined&&(!nonnegative(m.releasedSimAt)||m.releasedSimAt>s.elapsed))return false;
  ...
  for(const side of ['player','enemy'] as const)if(r.reserve[side]+r.manifests.filter(m=>m.side===side&&!m.returning).length!==initialReserve+(s.operation?.endless?.generatedReserve[side]??0))return false;

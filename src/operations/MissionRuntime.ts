...
export function stepPhysicalMission(state:BattlefieldState,terrain:TerrainSystem):void {
  ...
  const satisfied=plan.kind==='line-defense'?holdsHouse&&supplied&&route&&attackRepelled:holdsHouse&&line&&supplied&&route;
  ...
  m.breachedFor=enemyHouse&&(plan.kind!=='breakthrough'||m.houseTaken)?m.breachedFor+dt:0;
  ...
  const minimum=plan.kind==='line-defense'?2:5;
  ...
  else if(m.securedFor>=12)finish('victory',plan.kind==='line-defense'?'The attack withdrew. Your billet remains occupied and physically supplied.':'The house, field position and supply road form a sustained foothold.');

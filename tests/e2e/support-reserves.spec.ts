...
async function support(page:Page){
  ...
  if(!await page.locator('.support-other').evaluate(el=>(el as HTMLDetailsElement).open))await page.locator('.support-other>summary').click();
}
...
for(const reserves of [0,24,48] as const)test(`Open Front shows the configured ${reserves}-person reserve`,async({page},testInfo)=>{
  ...
  await support(page);await expect(page.locator('.rescue-reviews')).toContainText(`Reserve ${reserves}/${reserves}`);
  ...
  await support(page);await expect(page.locator('.rescue-reviews')).toContainText('Reserve 16/24');
  ...
  await expect(page.locator('.rescue-reviews')).toContainText('Reserve 16/24');
